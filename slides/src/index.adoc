...
// nest++
using PersonEntity = AllOf<Name, Role>;
// nest--

// nest++
using PersonSet = EntitySet<PersonId, PersonEntity>;
// nest--
----
--
...
void testPersonCreate() {
    // nest++
    auto repo = ToRepository<PersonSet>{};
    // nest--

    // nest++
    auto cmd = EntityCreate<PersonEntity>{ 
        Name{"Bjarne Stroustrup"},
        Role::Teacher };
    // nest--

    // nest++
    to_command_processor<PersonSet>(cmd, repo);
    // nest--
}
// nest--
----
...
// nest++
    -> std::map<Id, ToRepository<Entity>>;
// nest--
----
--
...
// nest++
template<class T>
constexpr auto to_command_processor = 
    toCommandProcessor(ADL{}, Ptr<T>{});
// nest--
...
constexpr auto toCommandProcessor(ADL, EntitySet<Id, Entity> *) {
    ...
    return [](const ToCommand<T>& cmd, ToRepository<T>& repo) {
        ...
        oneVisit(cmd,
            // nest++
            [&](const EntityCreate<Entity>& create) {
                repo[repo.createId()] = create;
            },
            // nest--
            // nest++
            [&](const EntityUpdate<Id, Entity>& update) {
                auto [id, entityCmd] = update;
                to_command_processor<Entity>(entityCmd, repo[id]);
            },
            // nest--
            // nest++
            [&](const EntityDestroy<Id>& id) {
                repo.destroy(id);
            });
            // nest--
       // nest--
